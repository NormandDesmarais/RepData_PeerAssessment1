...
The maximum number of steps during the day (=`r max_steps_interval`) occurs 
within the interval `r max_interval` minutes (i.e. around `r max_int_str`).
...
```
The dataset contains `r nb_miss` missing values, which counts for
`r miss_perc`% of the observables.
...
```{r, compare_dataset3, results="asis"}
